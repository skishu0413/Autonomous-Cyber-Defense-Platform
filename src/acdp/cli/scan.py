"""CLI entry point for ``python -m acdp scan``.

Loads the platform configuration, starts the Scheduler / Red Team connector,
and blocks until SIGINT or SIGTERM, then performs a graceful shutdown.

Usage::

    python -m acdp scan [--config PATH]
...
def main(argv: list[str] | None = None) -> int:
    """Entry point for ``python -m acdp scan``.
    ...
    parser = argparse.ArgumentParser(
        prog="acdp scan",
        description="Start the Scheduler / Red Team probe connector.",
    )
    parser.add_argument(
        "--config",
        metavar="PATH",
    ...
    print_banner()
    print_success(f"Scheduler connector starting  (config: {config_path})")
