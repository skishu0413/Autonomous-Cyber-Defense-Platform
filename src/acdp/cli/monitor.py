"""CLI entry point for ``python -m acdp monitor``.

Loads the platform configuration, starts the Log Stream Connector, and blocks
until SIGINT or SIGTERM, then performs a graceful shutdown.

Usage::

    python -m acdp monitor [--config PATH]
...
def main(argv: list[str] | None = None) -> int:
    """Entry point for ``python -m acdp monitor``.
    ...
    parser = argparse.ArgumentParser(
        prog="acdp monitor",
        description="Start the Log Stream Connector.",
    )
    parser.add_argument(
        "--config",
        metavar="PATH",
    ...
    print_banner()
    print_success(f"LogStreamConnector starting  (config: {config_path})")
